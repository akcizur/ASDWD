...
const mobileViewModes = [
  ...
  { id: 'grid', icon: 'fa-th-large' },
  { id: 'magazine', icon: 'fa-newspaper' },
  { id: 'compact', icon: 'fa-list-ul' },
];
        ...
        <div className="flex md:hidden items-center bg-neutral-100 dark:bg-neutral-900 p-1 rounded-lg text-xs">
          {mobileViewModes.map((mode) => (
            <button
              ...
              aria-label={`Přepnout na ${mode.id} zobrazení`}
              aria-pressed={viewMode === mode.id}
              className={'focus-ring px-2 py-1 rounded-lg transition-all ' + (
                viewMode === mode.id ? 'bg-white dark:bg-neutral-800 shadow-sm' : ''
              )}
            >
